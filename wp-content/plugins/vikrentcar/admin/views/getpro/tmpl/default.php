...
function vikwpStartDownload() {
	...
	jQuery.ajax({
		type: "POST",
		url: "admin.php",
		data: { option: "com_vikrentcar", task: "license.downloadpro", key: "<?php echo $lic_key; ?>" }
	}).done(function(res) {
...
function vikwpStopMonitoring(result) {
	...
	if (result === true) {
		var continuebtn = '<p><button type="button" class="button button-primary" onclick="document.location.href=\'admin.php?option=com_vikrentcar\';"><?php echo addslashes(JText::_('VRCUPDCOMPLOKCLICK')); ?></button></p>';
		jQuery('.vikwppro-download-result').html('<h1 class="vikwp-download-success"><?php VikRentCarIcons::e('check'); ?></h1>'+continuebtn);
	} else {
		var continuebtn = '<p><button type="button" class="button" onclick="document.location.href=\'admin.php?option=com_vikrentcar&view=gotopro\';"><?php echo addslashes(JText::_('VRCUPDCOMPLNOKCLICK')); ?></button></p>';
