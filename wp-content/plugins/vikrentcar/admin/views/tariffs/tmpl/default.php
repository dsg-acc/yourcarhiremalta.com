...
if (!empty($prices)) {
?>
<div class="vrc-admin-wizard-container">
	<div class="vrc-admin-wizard-inner">
	<?php
	if (!empty($this->rows)) {
		?>
		<a class="btn vrc-wizard-btn" href="index.php?option=com_vikrentcar"><?php VikRentCarIcons::e('home'); ?> <?php echo JText::_('VRCMENUDASHBOARD'); ?></a>
		<?php
	}
	?>
		<a class="btn vrc-wizard-btn" href="javascript: void(0);" onclick="showVrcWizard();"><?php VikRentCarIcons::e('magic'); ?> <?php echo JText::_('VRCTOGGLEWIZARD'); ?></a>
	</div>
</div>
<?php
}
					...
					<div class="vrc-quickres-head">
						...
						<div class="vrc-quickres-head-right">
							<form name="vrchcar" method="post" action="index.php?option=com_vikrentcar">
								...
								foreach ($allc as $cc) {
									?>
									<option value="<?php echo $cc['id']; ?>"<?php echo $cc['id'] == $idcar ? ' selected="selected"' : ''; ?>><?php echo $cc['name']; ?></option>
									<?php
								}
								?>
								</select>
					...
					<div class="vrc-param-container">
						...
						<div class="vrc-param-setting">
							<div class="vrc-fares-tabs">
								<div class="dailypricesactive"><?php echo JText::_('VRCDAILYFARES'); ?></div>
								...
								<div class="hourlyprices">
									<a href="index.php?option=com_vikrentcar&task=tariffshours&cid[]=<?php echo $idcar; ?>"><?php echo JText::_('VRCHOURLYFARES'); ?></a>
								</div>
							</div>
						<?php
						if (empty($prices)) {
						...
						?>
							<form name="newd" method="post" action="index.php?option=com_vikrentcar" onsubmit="javascript: if (!document.newd.ddaysfrom.value.match(/\S/)){alert('<?php echo addslashes(JText::_('VRMSGTWO')); ?>'); return false;} else {return true;}">
								<div class="vrc-insertrates-cont">
									<div class="vrc-insertrates-top">
										<div class="vrc-ratestable-lbl"><?php echo JText::_('VRDAYS'); ?></div>
									...
									<div class="vrc-insertrates-bottom">
										<div class="vrc-ratestable-lbl"><?php echo JText::_('VRDAILYPRICES'); ?></div>
								...
								<div class="vrc-insertrates-save">
									<input type="submit" class="btn vrc-config-btn" name="newdispcost" value="<?php echo JText::_('VRINSERT'); ?>"/>
									<input type="hidden" name="cid[]" value="<?php echo $idcar; ?>"/>
									<input type="hidden" name="task" value="tariffs"/>
								</div>
							</form>
				...
				if (empty($rows)) {
					...
					<form action="index.php?option=com_vikrentcar" method="post" name="adminForm" id="adminForm" class="vrc-list-form">
						<div class="vrc-tariffs-updaterates-cont">
							<input type="submit" name="modtar" value="<?php echo JText::_('VRPVIEWTARTWO'); ?>" onclick="vrRateSetTask(event);" class="btn vrc-config-btn" />
						</div>
						...
						<input type="hidden" name="option" value="com_vikrentcar" />
						<input type="hidden" name="task" id="vrtask" value="tariffs" />
