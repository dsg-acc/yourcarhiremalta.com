...
<form action="index.php?option=com_vikrentcar&task=orders" method="post" name="adminForm" id="adminForm" class="vrc-allorders-fm">
	...
	<div id="filter-bar" class="btn-toolbar vrc-btn-toolbar" style="width: 100%; display: inline-block;">
		<div class="btn-group pull-left input-append">
			<input type="text" name="filtnc" id="filtnc" autocomplete="off" placeholder="<?php echo JText::_('VRCFILTCNAMECNUMB'); ?>" value="<?php echo (strlen($filtnc) > 0 ? $filtnc : ''); ?>" size="30" />
		...
		?>
		<div class="btn-group pull-left input-append">
			<input type="text" id="customernominative" autocomplete="off" placeholder="<?php echo JText::_('VRCUSTOMERNOMINATIVE'); ?>" value="<?php echo $cust_id_filter ? htmlspecialchars($rows[0]['customer_fullname']) : ''; ?>" size="30" />
		...
		<div id="vrc-search-tools-cont" class="js-stools-container-filters clearfix" style="display: none;">
			...
			<div class="btn-group pull-left" id="vrc-dates-cont" style="display: <?php echo (!empty($pdatefilt) && (!empty($pdatefiltfrom) || !empty($pdatefiltto)) ? 'inline-block' : 'none'); ?>;">
				<input type="text" id="vrc-date-from" placeholder="<?php echo JText::_('VRNEWSEASONONE'); ?>" value="<?php echo $pdatefiltfrom; ?>" size="10" name="datefiltfrom" />&nbsp;-&nbsp;<input type="text" id="vrc-date-to" placeholder="<?php echo JText::_('VRNEWSEASONTWO'); ?>" value="<?php echo $pdatefiltto; ?>" size="10" name="datefiltto" />
			</div>
			<div class="btn-group pull-left">
	...
	<div class="table-responsive">
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++) {
			...
			<tr class="row<?php echo $kk; ?>">
				...
				<td class="center">
					<a class="vrc-orderid" href="index.php?option=com_vikrentcar&amp;task=editorder&amp;cid[]=<?php echo $row['id']; ?>"><?php echo $row['id']; ?></a>
				</td>
				<td>
					<a class="vrc-orderslist-viewdet-link" href="index.php?option=com_vikrentcar&amp;task=editorder&amp;cid[]=<?php echo $row['id']; ?>">
				...
				<td class="center">
					<?php echo $invoice_icon.(!empty($row['adminnotes']) ? '<span class="hasTooltip vrc-admin-tipsicon" title="'.htmlentities(nl2br($row['adminnotes'])).'"><i class="fas fa-comment-dots"></i></span>' : ''); ?>
				</td>
				<td class="center">
					<?php echo $status_lbl; ?>
				</td>
