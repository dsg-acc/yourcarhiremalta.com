...
class VikRentCarCustomersPin {
	...
	*/
	private function setPinCookie($pin) {
		...
		if (!empty($pin)) {
			...
			$cookie = JFactory::getApplication()->input->cookie;
			$cookie->set('vrcPinData', $pin_cookie, (time() + (86400 * 365)), '/', '', false, true);
		}
	...
	*/
	public function getCustomerByID($cust_id) {
		$customer = array();
		if (!empty($cust_id)) {
			$q = "SELECT * FROM `#__vikrentcar_customers` WHERE `id`=".$this->dbo->quote($cust_id).";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
			if ($this->dbo->getNumRows() == 1) {
				$customer = $this->dbo->loadAssoc();
	...
	*/
	public function getCustomerFromBooking($orderid) {
		$customer_id = 0;
		if (!empty($orderid)) {
			$q = "SELECT `idcustomer` FROM `#__vikrentcar_customers_orders` WHERE `idorder`=".(int)$orderid.";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
			if ($this->dbo->getNumRows() == 1) {
