...
class VikRentCarTracker
{
	...
	public function getFingerprint()
	{
		...
		if (!empty($sesstfp)) {
			...
			$cookie->set('vrcTFP', static::$fingerprint, (time() + (86400 * 365)), '/');
			return $sesstfp;
		}
	...
	protected function getReferrer()
	{
		...
		if (!empty($sess_referrer)) {
			...
			if (!empty($provenience)) {
				...
				$cookie->set('vrcTProv', $provenience, floor((time() + (86400 * (float)self::loadSettings('trkcookierfrdur')))), '/');
			}
	...
	public function closeTrack()
	{
		...
		if (!$track_closed) {
			...
			if (!(int)self::loadSettings('trkenabled') || JFactory::getApplication()->isAdmin()) {
				return 0;
			}
