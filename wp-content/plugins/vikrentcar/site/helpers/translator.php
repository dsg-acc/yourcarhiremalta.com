...
class VikRentCarTranslator {
	...
	public function __construct() {
		...
		$this->translations_path_file = VRC_ADMIN_PATH.DS.'fields'.DS.'translations.xml';
	...
	*/
	public function getTranslatedTable($table, $lang) {
		$translated = array();
		$q = "SELECT * FROM `#__vikrentcar_translations` WHERE `table`=".$this->dbo->quote($this->replacePrefix($table))." AND `lang`=".$this->dbo->quote($lang)." ORDER BY `#__vikrentcar_translations`.`reference_id` ASC;";
		$this->dbo->setQuery($q);
		$this->dbo->execute();
		if ($this->dbo->getNumRows() > 0) {
			$records = $this->dbo->loadAssocList();
	...
	public function translateContents(&$content, $table, $alias_keys = array(), $ids = array(), $lang = null) {
		...
		if (!(count($translated) > 0)) {
			...
			$q = "SELECT * FROM `#__vikrentcar_translations` WHERE `table`=".$this->dbo->quote($this->replacePrefix($table))." AND `lang`=".$this->dbo->quote($to_lang). (count($ids) > 0 ? " AND `reference_id` IN (".implode(",", $ids).")" : "") .";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
			if ($this->dbo->getNumRows() > 0) {
				$records = $this->dbo->loadAssocList();
	...
	*/
	...
	private function allowMultiLanguage($skipsession = false) {
		if (!class_exists('vikrentcar')) {
			require_once(VRC_SITE_PATH . DS . "helpers" . DS ."lib.vikrentcar.php");
		}
		return VikRentCar::allowMultiLanguage($skipsession);
	}
